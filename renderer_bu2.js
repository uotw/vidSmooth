...
if (ismac) {
    ...
    var winoriginal;
    appRootDir = unixify(appRootDir);
    var ffmpegpath = appRootDir + '/node_modules/ffmpeg/ffmpeg.exe';
    var ffprobepath = appRootDir.replace(/(\s+)/g, '\\$1') + '/node_modules/ffprobe/ffprobe.exe';
    var sendkeysbatpath = appRootDir + '/sendKeys.bat';
    var temporiginal = workdir + '/temp.mp4';
    var workdir = unixify(ostemp) + '/' + maketemp();
...
function setupselect() {
    return () => new Promise((resolve, reject) => {
        if(ismac){
            ...
            var infile = filelist[0].replace(/(\s+)/g, '\\$1');
         }
...
function preview() {
    ...
    if (!fs.existsSync(outfile)) {
        if (canplay != "") {
            ...
            // } else {
            //     myqueue.push(customSpawn('cmd.exe', ['/c', '"' + '"' + ffmpegpath + '"' + '"', '-i', filelist[0], '-an', '-y', '-vf', vftext, outfile], {
            //         windowsVerbatimArguments: true
            //     }));
            // }
...
$('#fullvid').click(function() {
    ...
    // } else {
    //     myqueue.push(customSpawn('cmd.exe', ['/c', '"' + '"' + ffmpegpath + '"' + '"', '-i', filelist[0], '-y', '-vf', transformvf, '-f', 'null', '-'], {
    //         windowsVerbatimArguments: true
    //     }));
    //     myqueue.push(customSpawn('cmd.exe', ['/c', '"' + '"' + ffmpegpath + '"' + '"', '-i', filelist[0], '-y', '-crf', '17', '-vf', deshake, finalFile], {
    //         windowsVerbatimArguments: true
    //     }));
    // }
    myqueue.push(progress(1));
